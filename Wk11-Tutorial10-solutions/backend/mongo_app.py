...
@app.get("/todos")
async def db_get_todos(current_user: str = Depends(get_current_user)):
    return todos_collection.find({"user_id": current_user}, {"_id": 0}).to_list()


# --- Create a new todo item, associating it with the current user ---
@app.post("/todos")
async def db_create_todo(todo: dict, current_user: str = Depends(get_current_user)):
    todo["user_id"] = current_user  # Set the user_id field to the current user
...
async def db_delete_todo(todo_id: str, current_user: str = Depends(get_current_user)):
    result = todos_collection.delete_one({"id": todo_id, "user_id": current_user})
...
async def db_update_todo(
    todo_id: str, todo_obj: dict, current_user: str = Depends(get_current_user)
):
    result = todos_collection.find_one_and_update(
        {"id": todo_id, "user_id": current_user},
        {"$set": todo_obj},
